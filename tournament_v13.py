...
GLOBAL_PATHS = {
    ...
    'V12 Preemptive': r'g:/po/kaggriculture/kaggriculture_v12_preemptive_apex.py',
    'V13 Infinite': r'g:/po/kaggriculture/kaggriculture_v13_infinite_horizon.py'
}
...
if __name__ == '__main__':
    ...
    # 5 random seeds to keep total matches at 100
    seeds = [42, 100, 2026, 7777, 9999]
    tasks = []
    ...
    print(f'Starting V13 Tournament ({len(tasks)} matches total)...')
    t0 = time.time()
    ...
    hdr = f"{'':16}" + ''.join([f"{n:>15}" for n in names])
    print(hdr)
    for n1 in names:
        row = f"{n1:15} "
        for n2 in names:
