"""Adaptive multi-route Kaggriculture controller with execution guards."""
import base64
import copy
import json
import math
...
def _lead_horizon(obs):
    shops = list((_get(_get(obs, "town", {}) or {}, "unlocked_shops", []) or []))
    first = shops[0] if shops else None
    # The lean two-turn queue lead is strongest in broad crop / mixed demand
    # openings.  Yarn and bakery openings keep the proven one-turn timing.
    return 1 if first in {"YARN_STORE", "BAKERY"} else 2

def _future_sells(obs, step):
    horizon_limit = 15
    for h in range(1, horizon_limit + 1):
        if step + h >= len(_kawa_actions(obs)):
            break
        result = {}
        found_any = False
        for raw in (_kawa_actions(obs)[step + h].get("market") or []):
...
def _preempt_shift(obs, action, step):
    ...
    if state.get("due") or _clone_distance(obs) > _PREEMPT_MAX_CLONE_DISTANCE:
        return action
    future, best_horizon = _future_sells(obs, step)
    if not future or best_horizon == 0:
        return action
    ...
    if shifted:
        ...
        state["due_step"] = step + best_horizon
        state["due"] = shifted
    return action
