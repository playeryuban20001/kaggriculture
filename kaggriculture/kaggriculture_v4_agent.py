...
def agent(obs):
    ...
    # Count animals, coops, weeds, and empty tiles
    coops = []
    animals = []
    empty = []
    weeds = []
    ...
    # 6. Sort by Manhattan distance to the central shed (4,4)
    # This ensures we build pastures closest to the shed first, minimizing walk distance
    empty.sort(key=lambda pos: distance(pos[0], pos[1], 4, 4))
    animals.sort(key=lambda a: distance(a[0], a[1], 4, 4))
    weeds.sort(key=lambda pos: distance(pos[0], pos[1], 4, 4))
    ...
    # 2. Smarter Land Expansion (Stop expanding in late game)
    if farm["money"] > 3500 and len(empty) == 0 and len(weeds) == 0 and len(farm.get("unlocked_quadrants", [])) < 4 and step < 500:
        market_orders.append(["BUY_LAND"])
        
    # 4. Optimize Hire Ratio (1 worker can easily feed 2 cows per day)
    target_hands = (num_animals // 2) + 1
    ...
    # Buy Cow (Stop buying in late game, as a 2500 investment won't pay off in < 5 days)
    if len(coops) > 0 and shed.get("COW", 0) == 0 and farm["money"] >= 2500 and step < 550:
        market_orders.append(["BUY_ANIMAL", "COW", 1])
        
    # 1. Dynamic Market Manipulation Threshold
    # Only pump the market if we have enough cows to produce real volume, or we're swimming in cash
    if num_animals >= 5 or farm["money"] > 5000:
        if step % 2 == 0:
    ...
    # 5. Smart Selling (Hold if prices crash)
    sell_thresholds = {
        "MILK": 180,  # Base 250, wait if price crashes too hard
        "EGG": 30,    # Base 50
        "WOOL": 180   # Base 250
    }
    ...
    for product in ["MILK", "EGG", "WOOL"]:
        ...
        if count > 0:
            ...
            # Only sell if price is acceptable, or if it's the very last day (step > 690) to liquidate
            if current_price >= sell_thresholds.get(product, 0) or step > 690:
    ...
    for i, w_pos in enumerate(workers):
        ...
        # 2. Animal Caretakers (Worker 1+)
        # Each worker handles 2 animals based on their index (Worker 1 handles animals 0 and 1)
        hand_idx = i - 1
        animal_start = hand_idx * 2
        
        handled = False
        for offset in range(2):
            animal_idx = animal_start + offset
            if animal_idx < len(animals):
                tx, ty, tile_data = animals[animal_idx]
                ...
                if not tile_data.get("fed_today", False) or tile_data.get("fertilizer_available", False) or tile_data.get("yield_units", 0) > 0 or not tile_data.get("cared_today", False):
