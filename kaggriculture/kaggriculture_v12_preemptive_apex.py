...
def _future_sells(obs, step):
    horizon = _lead_horizon(obs)
    if step + horizon >= len(_kawa_actions(obs)):
        return {}
    result = {}
    for raw in (_kawa_actions(obs)[step + horizon].get("market") or []):
...
def _preempt_shift(obs, action, step):
    ...
    if shifted:
        ...
        state["due_step"] = step + _lead_horizon(obs)
        state["due"] = shifted
    return action
